...
from scinexus.warning import deprecated_args, deprecated_callable
...
def test_function_deprecated_args_deprecated_callable_chained_decorators(recwarn):
    ...
    got = changed(x=5, b=3)
    assert got == 5
    warn_msgs = [warning.message.args[0] for warning in recwarn.list]
    assert any("argument x will be removed" in w for w in warn_msgs)
    assert any("argument b is discontinued" in w for w in warn_msgs)
    assert any("function changed is discontinued" in w for w in warn_msgs)
...
def test_class_deprecated(recwarn):
    ...
    fooclass()
    warn_msgs = [warning.message.args[0] for warning in recwarn.list]
    assert any("fooclass is discontinued" in w for w in warn_msgs)
