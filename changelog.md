...
- A writable `DataStoreSqlite` is created only by the master process, matching `DataStoreDirectory`, which has always returned early off the master. Construction off the master still succeeds, and the first read or write that reaches a database which does not exist, or a file carrying no schema, now raises `OSError` instead of creating the database and its tables. So an app that opens a writable store of its own inside a worker no longer works, as it already did not for a directory store. Using a store the master made is unchanged, and an in-memory store is exempt: it has no file to race over, so refusing would remove it outright for no gain.
