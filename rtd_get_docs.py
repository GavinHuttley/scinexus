...
def get_latest_run(workflow_filename: str, headers: dict) -> dict:
    url = f"https://api.github.com/repos/cogent3/scinexus/actions/workflows/{workflow_filename}/runs"
...
def wait_for_run_completion(run: dict, headers: dict) -> dict:
    run_id = run["id"]
    run_url = f"https://api.github.com/repos/cogent3/scinexus/actions/runs/{run_id}"
...
def download_and_extract_artifact(run: dict, headers: dict) -> None:
    artifact_name = "scinexus-docs-html"
    response = download_extract_artifact(
        run, "artifacts_url", headers, "GitHub API request failed with status "
    )
    try:
...
def download_extract_artifact(arg0, arg1, headers, arg3):
    artifacts_url = arg0[arg1]

    result = requests.get(artifacts_url, headers=headers, timeout=10)

    if not result.ok:
        msg = f"{arg3}{result.status_code}: {result.text[:200]}"
        raise RuntimeError(msg)
